...
class TextSanitizer(sanitizer.HTMLSanitizer):
    ...
    allow_token_parsers = []  # type:ignore
    ...
    def allowed_token(self, token, token_type):
        ...
        for data in token["data"]:
            for allow_token_parser in self.allow_token_parsers:
                if (
                    allow_token_parser().parse(data[0], data[1])
                    and data[0] not in allowed_attributes
                ):
                    sanitizer.HTMLSanitizer.allowed_attributes.append(data[0])
                    data_found = True
        allowed_token_res = super().allowed_token(token, token_type)
        if data_found:
